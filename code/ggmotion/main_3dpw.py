import argparse
import torch
from utils.data_loader import Pose3dPW3D
...
def run():
    ...
    loaders_test = {}
    len_test = 0
    for act in Pose3dPW3D.actions:
        dataset_test = Pose3dPW3D(args.data, actions=act, input_n=args.past_length, output_n=args.future_length,
    ...
    if args.mode == 'train':
        ...
        dataset_train = Pose3dPW3D(args.data, actions="all", input_n=args.past_length, output_n=args.future_length,
        ...
        work_dir, log = runtime.exp_create(args.exp, Pose3dPW3D.exp, args.cfg)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch % args.eval_interval == 0:
                if args.future_length > 12:
                    eval_frame = [(2, 100), (5, 200), (11, 400), (17, 600), (23, 800), (29, 1000)]
                else:
                    eval_frame = [(2, 100), (5, 200), (11, 400)]

                print(f">>> eval Epoch: {epoch} Lr: {lr:.7f} Pri_loss: {pri_loss:.7f}, Aux_loss: {aux_loss:.7f} Manner: {args.manner}", file=log, flush=True)
                avg_mpjpe, avg_avg_mpjpe, eval_msg = runtime.test_3dpw(model, eval_frame, Pose3dPW3D.actions,
                ...
                runtime.save_ckpt(work_dir, Pose3dPW3D.exp, state, epoch, avg_avg_mpjpe, args.keep)
    elif args.mode == 'eval':
        ...
        _, _, eval_msg = runtime.test_3dpw(model, eval_frame, Pose3dPW3D.actions, loaders_test, dataset_test.dim_used, args.scale)
        print(eval_msg)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data', type=str, default='./data/3dpw', help='path to 3dpw dataset')
    parser.add_argument('--seed', type=int, default=1234567890, help='random seed')
    parser.add_argument('--cfg', type=str, default='cfg/3dpw_short.yml', help='path to the configuration in .yml')
    ...
    parser.add_argument('--manner', type=str, default='all', help='all')
