import argparse
import torch
from utils.data_loader import H36motion3D
...
def run():
    ...
    loaders_test = {}
    len_test = 0
    for act in H36motion3D.define_acts(args.act):
        dataset_test = H36motion3D(args.data, actions=act, input_n=args.past_length, output_n=args.future_length,
    ...
    if args.mode == 'train':
        ...
        dataset_train = H36motion3D(args.data, actions="all", input_n=args.past_length, output_n=args.future_length,
        ...
        work_dir, log = runtime.exp_create(args.exp, H36motion3D.exp, args.cfg)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch % args.eval_interval == 0:
                ...
                avg_mpjpe, avg_avg_mpjpe, eval_msg = runtime.test(model, eval_frame, H36motion3D.actions,
                ...
                runtime.save_ckpt(work_dir, H36motion3D.exp, state, epoch, avg_avg_mpjpe, args.keep)
    elif args.mode == 'eval':
        ...
        _, _, eval_msg = runtime.test(model, eval_frame, H36motion3D.actions, loaders_test, dataset_test.dim_used, args.scale)
        print(eval_msg)
        ...
        if args.future_length > 12:
            eval_frame = [(1, 80), (3, 160), (5, 240), (7, 320), (9, 400), (11, 480), (13, 560), (15, 640), (17, 720), (19, 800), (21, 880), (23, 960), (24, 1000)]
        else:
            eval_frame = [(1, 80), (3, 160), (5, 240), (7, 320), (9, 400)]

        viz_msg = runtime.viz_test(model, eval_frame, args.act, args.seq, loaders_test, dataset_test.dim_used, args.viz, H36motion3D.exp, scale=args.scale)
        print(viz_msg)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfg', type=str, default='cfg/h36m_short.yml', help='path to the configuration in .yml')
