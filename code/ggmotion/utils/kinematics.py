...
def fkl_torch_with_rot(angles, parent, offset, rotInd, expmapInd):
    """
    Batched FK returning both joint positions and global joint rotations.

    This mirrors fkl_torch exactly for positions so existing CMU numbers remain
    comparable, but also exposes the global rotation matrices needed for
    pose-level dual-quaternion preprocessing.
    """
    n = angles.data.shape[0]
    j_n = offset.shape[0]
    p3d = torch.from_numpy(offset).float().unsqueeze(0).repeat(n, 1, 1).to(DEVICE)
